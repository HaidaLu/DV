...
pub fn setup(seed: [u8; 48]) -> (Vec<u8>, Vec<u8>){
    use core::ops::Mul;
...
pub fn extract(ancestor: Vec<u8>, id: Vec<Vec<u8>>) -> Vec<u8> {
    vec![0]
}

pub fn encrypt(params: Vec<u8>, message: Vec<u8>, id: Vec<Vec<u8>>) -> (Vec<u8>, Vec<u8>) {

}

pub fn decrypt(entity: Vec<u8>, c1: Vec<u8>, c2: Vec<u8>) -> Vec<u8> {
