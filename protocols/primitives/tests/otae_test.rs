...
fn test_aes256_cbc_encrypt_then_decrypt_gives_plaintext() {
    ...
    let ct = primitives::encrypt_aes_256_cbc(message.as_bytes(), &key, &iv).ok().unwrap();
    let pt = primitives::decrypt_aes_256_cbc(&ct[..], &key, &iv).ok().unwrap();
...
fn test_aes256_ctr_encrypt_then_decrypt_gives_plaintext() {
    ...
    let cipher_text = primitives::encrypt_aes_256_ctr(message.as_bytes(), &key, &ad).ok().unwrap();
    let decrypted_data=primitives::decrypt_aes_256_ctr(&cipher_text[..], &key, &ad).ok().unwrap();
