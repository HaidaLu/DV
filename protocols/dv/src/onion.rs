...
pub fn send(s : &mut Vec<String>, ad: &[u8], msg: Vec<u8>, iv: &[u8]) -> (Vec<String>, String) {

//pub fn send(ss : SenderState, ad: &[u8], msg: Vec<u8>, iv: &[u8]) -> (String, String) {
    ...
    let mut c = Vec::new();
    let cn = primitives::encrypt_aes_256_cbc(pt.as_bytes(), &k, &iv).expect("unable to encrypt the plaintext!");
    c.insert(0, cn);
...
pub fn receive(s : & mut Vec<String>, ad: &[u8], ct: String, iv: &[u8]) -> (Vec<String>, Vec<u8>) {
//pub fn receive(rs : ReceiverState, ad: &[u8], ct: String, iv: &[u8]) -> (String, Vec<u8>) {
    ...
    let pt = String::from_utf8(primitives::decrypt_aes_256_cbc(&last_ciphertext, &k, &iv).expect("unable to encrypt the plaintext!")).unwrap();
