...
fn send_and_receive() {
    ...
    let cn = primitives::encrypt_aes_256_cbc(pt.as_bytes(), &k, &iv).expect("unable to encrypt the ciphertext! ");
    c_lists.insert(0, cn);
    ...
    let pt2 =  primitives::decrypt_aes_256_cbc(&last_ciphertext, &k, &iv).unwrap();
...
fn test_uni_send_receive() {
    ...
    loop {
        ...
        let (mut _new_s_state, ct2) = onion::send(&mut _s_state, &ad, Vec::from(message), &iv);
        let (mut _new_r_state, pt2) = onion::receive(&mut _r_state, &ad, ct2, &iv);
...
fn test_read_file () {
    ...
    let (s3, r3) = dv::onion::init();
    let (s4, r4) = dv::onion::init();
    let s_state = vec![s1, s2, s3, s4];
    let r_state = vec![r1, r2, r3, r4];
    let ad = dv::generate_iv();
    let iv = dv::generate_iv();
