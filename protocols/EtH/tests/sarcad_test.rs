...
fn send_hundred_same_messages() {
    ...
    loop {
        let (_new_st_a, ct) = EtH::send(st_a, &iv, pt);
        let (_new_st_b, pt) = EtH::receive(st_b, &iv, ct.as_slice());
        let crypt_message = str::from_utf8(pt.as_slice()).unwrap();
        assert_eq!(message, crypt_message);
        println!("{} :", i);
        println!("Alice send: {}", message);
...
fn send_hundred_random_messages() {
    ...
    loop {
        ...
        let (st_a, ct) = EtH::send(st_a, &iv, original_message.as_bytes());
        let (st_b, pt) = EtH::receive(st_b, &iv, ct.as_slice());
        let crypt_message = str::from_utf8(pt.as_slice()).unwrap();
...
fn test_read_file_send_and_receive(){
    ...
    let (st_a, ct) = EtH::send(st_a, &iv, message.as_bytes());
    let (st_b, pt) = EtH::receive(st_b, &iv, ct.as_slice());
    let crypt_message=str::from_utf8(pt.as_slice()).unwrap();
        ...
        for line in lines {
            if let Ok(message) = line {
                if i % 2 == 0 {
                    ...
                    let (bs_st_a, ct) = EtH::send(bs_st_a, &iv2, message.as_bytes());
                    let(ar_st_b, pt) = EtH::receive(ar_st_b, &iv2, ct.as_slice());
                    let crypt_message=str::from_utf8(pt.as_slice()).unwrap();
